...
import {
  ...
  SprayCan, Sparkles, ChevronRight, Plus, Thermometer as TempIcon,
} from "lucide-react";
...
type NavLeaf = { title: string; url: string; icon: React.ElementType; mod?: ModuleName; desc?: string };
...
const quickActions = [
  { title: "Log a temp", url: "/temperatures", icon: TempIcon, color: "bg-blue-500" },
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-card rounded-t-2xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b">
          ...
          <button onClick={onClose} className="h-7 w-7 flex items-center justify-center rounded-full bg-muted text-muted-foreground">
            <X className="h-4 w-4" />
          </button>
        </div>
        {/* Items */}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn("text-sm font-semibold", active ? "text-primary" : "text-foreground")}>{item.title}</p>
...
function FAB() {
  ...
  return (
    <>
        ...
        <>
          ...
          <div className="fixed bottom-24 right-4 z-50 md:hidden space-y-2">
            {quickActions.map((action) => (
              ...
              >
                <div className={cn("h-8 w-8 rounded-xl flex items-center justify-center shrink-0", action.color)}>
      ...
      >
        <Plus className={cn("h-6 w-6 transition-colors", open ? "text-foreground" : "text-primary-foreground")} />
      </button>
    </>
  );
}
...
function MoreSheetExtras({ onClose }: { onClose: () => void }) {
  ...
  const orgItems: NavLeaf[] = [
    ...(isHQ && role.isManager ? [{ title: "HQ Dashboard", url: "/hq", icon: Building2, desc: "Multi-site overview" }] : []),
    ...(isOrgOwner ? [{ title: "Account & Billing", url: "/account", icon: CreditCard, desc: "Subscription & invoices" }] : []),
    ...(role.canViewSettings && hasSelectedSite ? [{ title: "Settings", url: "/settings", icon: Settings, desc: "Site, users, modules" }] : []),
  ];
  ...
  return (
    <div className="mt-2 border-t pt-2">
      <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground px-3 mb-2">Organisation</p>
      {orgItems.map((item) => (
        <button
          key={item.url}
