...
export function generateInspectionPackPdf(data: ReportData, aiNarrative?: string) {
  ...
  if (aiNarrative && aiNarrative.trim().length > 0) {
    doc.addPage(); header(doc, data, "AI Compliance Assessment");
    let ay = 28;
    doc.setFont("helvetica", "bold"); doc.setFontSize(14); doc.setTextColor(0, 0, 0);
    ...
    paragraphs.forEach((para) => {
      ...
      if (ay + lines.length * 5 > doc.internal.pageSize.getHeight() - 25) {
        doc.addPage(); header(doc, data, "AI Compliance Assessment");
        ay = 28;
      }
    ...
    if (ay + dlines.length * 4.5 > doc.internal.pageSize.getHeight() - 25) {
      doc.addPage(); header(doc, data, "AI Compliance Assessment");
      ay = 28;
    }
